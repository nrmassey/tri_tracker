...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		TCLAP::ValueArg<int> init_shp_arg("i", "init_shape", "Initialisation shape: 0 = ICOSAHEDRON, 1 = OCTAHEDRON", false, 0, "int", cmd);
		TCLAP::ValueArg<int> max_its_arg("I", "max_its", "Maximum number of iterations in equalisation phase", false, 1e3, "int", cmd);
		TCLAP::ValueArg<int> max_pts_arg("n", "max_pts", "Maximum number of points from the original grid allowed per triangle", false, 1, "int", cmd);
	...
	try
	{
		...
		tri_grid tg;
		tg.initialize(init_S == 0 ? ICOSAHEDRON : OCTAHEDRON, &nc_data, max_pts,
					  max_levs, max_I);
		tg.save(out_name);
